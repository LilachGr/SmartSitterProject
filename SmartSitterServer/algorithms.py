...
def check_time_availability(parameters):
    ...
    ans_date = db.run_select_query(query_date)
    if len(ans_date) != 0:
    ...
    ans_num_of_labs = db.run_select_query(query_num_of_labs)
...
def handle_login(parameters):
    ...
    ans_user_exist = db.run_select_query(query_user_exist)
    if len(ans_user_exist) != 0:
        return "error"
    query_email_exist = f"select * from users where email ='{email}'"
    ans_email_exist = db.run_select_query(query_email_exist)
